...
int main(int argc, char ** argv){
	...
	printf("  A questo punto, \"q\" e` ancora il nome di una stringa,\ne` perfettamente intercambiabile"
               "con \"char * u = c+lc+1;\"\ne corrisponde alla stringa di lunghezza %d: \"%s\"\n",
               (int)strlen(u), u);
	printf("Infine incrementando di nuovo \"q\" di \"strlen(u)+1\"=%d si supera anche il secondo\nvalore 0 contenuto"
               " in \"c\", e", (int)strlen(u)+1);
        q += strlen(u)+1;
	printf(" a questo punto, \"q\" non e` piu` il nome di una stringa,\nperche` nessuno degli "
		"elementi successivi dell'array \"q\" contiene il valore 0\natto a terminare una stringa.\n");
